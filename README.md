...
| `/locate mark <name>` / `/locate marks` / `/locate unmark <name>` | save the spot you stand on / list your marks / delete one; every line has the same clickable coordinates and buttons as search results, and dying auto-marks `death` (`deathMark` in the config turns that off server-wide) |
